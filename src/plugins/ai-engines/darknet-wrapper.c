	...
	if(dets && count > 0)
	{
		...
		//	int confirmed_classes_count = 0;
			...
			if(klass >= 0)
			{
				...
				box b = dets[i].bbox;
				//~ result->x = (b.x - b.w / 2.0) / (double)width;
				//~ result->y = (b.y - b.h / 2.0) / (double)height;
				//~ result->cx = b.w / (double)width;
				//~ result->cy = b.h / (double)height;
				...
				printf("result: bbox:{%.3f, %.3f, %.3f, %.3f}\n", 
